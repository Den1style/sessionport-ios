...
## 1. Собрать и прогнать тесты (обязательно)

```bash
cd iOS
xcodegen                      # регенерировать проект из project.yml
xcodebuild test -scheme SessionPort -destination 'platform=iOS Simulator,name=iPhone 16'
```

Если компилятор ругается — правки локализованы в 4 файлах:
